...
* Create named product types for items (eg: "Hot Drinks", "Juices")
...
* Each item in the store has a preparation time, defaulting to 4 minutes. Products can be edited to take longer.
* If an order has more than 4 items, add 6 minutes for every additional 4 items.
...
Administrators may put items or entire product types of products on sale. They can:
...
* After a product is added to and order, recommend three products other customers have ordered with this item. If there aren’t three other things that’ve been ordered, fill in with the most popular overall products
* Combinations of several items (ex: an espresso, latte, and bubble tea) with a small discount over ordering them individually
...
* "The order includes: an espresso, latte, and bubble tea. There are a total of 3 items."
